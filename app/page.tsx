...
// Avatar biểu tượng đầu người & thân người 100% SVG tối giản, vẽ chuẩn đẹp & thanh lịch
function renderClientAvatar(_sprite?: string, className = 'w-12 h-12') {
  return (
    <div className={`${className} rounded-full overflow-hidden bg-slate-100 dark:bg-slate-800/90 border border-slate-300/80 dark:border-slate-700 flex items-center justify-center shrink-0 shadow-xs`}>
      <svg
        viewBox="0 0 32 32"
      ...
      >
        ...
        {/* Shoulders / Body */}
        <path d="M 6.5 26.5 C 6.5 19.5, 25.5 19.5, 25.5 26.5 Z" />
      </svg>
    </div>
  );
}
...
export default function Home() {
  ...
  return (
    <main className={`min-h-screen font-sans antialiased transition-colors duration-300 ${
      ...
      {/* POPUP DRAWER "THÊM" (MORE) - TRƯỢT TỪ DƯỚI LÊN */}
      {isMobileMoreOpen && (
        <div className="md:hidden fixed inset-0 z-[60]">
          {/* Backdrop */}
          ...
          <div className={`absolute bottom-0 left-0 right-0 rounded-t-3xl border-t-[3px] border-[var(--line)] p-5 pb-[calc(2rem+env(safe-area-inset-bottom,0px))] space-y-3 animate-in slide-in-from-bottom duration-200 ${
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              {/* ADMIN BẢNG QUẢN TRỊ (DÀNH CHO ADMIN KHÁNG/ADMINS) */}
              {user && ADMIN_EMAILS.includes(user.email.toLowerCase()) && (
                <button
                  ...
                  className={`p-4 border-2 border-[var(--line)] rounded-2xl flex flex-col items-center gap-2 cursor-pointer transition-all active:scale-95 col-span-2 ${
                    activeTab === 'admin'
                      ? 'bg-purple-100 dark:bg-purple-950/50 shadow-[3px_3px_0_var(--line)] text-purple-700 dark:text-purple-300 font-bold'
                      : isDarkMode ? 'bg-slate-800 hover:bg-slate-700 shadow-[3px_3px_0_var(--line)]' : 'bg-white hover:bg-purple-50 shadow-[3px_3px_0_var(--line)] text-purple-700'
                  }`}
                >
                  {renderAIIcon('w-6 h-6 text-purple-600')}
                  <span className="text-[10px] font-black">Bảng Quản Trị Admin</span>
                </button>
              )}
            </div>
          </div>
