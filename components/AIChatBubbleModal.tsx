...
export default function AIChatBubbleModal({
  ...
  return (
    <>
      ...
      {!isOpen && (
        <div className="fixed bottom-[calc(60px+env(safe-area-inset-bottom,0px))] md:bottom-6 right-4 z-40 flex items-center gap-2">
          <button
            onClick={() => {
              setIsOpen(true);
              playSfx('click');
      ...
      {isOpen && (
        <div className="fixed bottom-[calc(64px+env(safe-area-inset-bottom,0px))] md:bottom-6 right-4 z-50 w-[calc(100vw-2rem)] sm:w-[380px] h-[520px] max-h-[75vh] flex flex-col border-3 border-[var(--line)] rounded-3xl shadow-[8px_8px_0_var(--line)] overflow-hidden animate-in fade-in zoom-in-95 duration-150 bg-white dark:bg-[#1e1e1e]">
          {/* Header */}
