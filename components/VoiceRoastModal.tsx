...
export function VoiceRoastModal({
  ...
  const evaluatePronunciation = (userSpeech: string) => {
    ...
    if (score >= 90) {
      ...
      levelClass = 'text-emerald-600 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/80 border-emerald-300 dark:border-emerald-700';
      playSfx('perfect');
    } else if (score >= 70) {
      roast = `Phát âm rất tốt! Nghe giọng ngọt ngào truyền cảm lắm, chỉ cần chút xíu âm điệu nữa là hoàn hảo!`;
      levelClass = 'text-blue-600 dark:text-blue-300 bg-blue-50 dark:bg-blue-950/80 border-blue-300 dark:border-blue-700';
      playSfx('success');
    } else if (score >= 45) {
      roast = `Cũng được đấy! Nhưng nghe âm điệu từ "${wordChinese}" hơi giống giọng miền Tây nói tiếng Trung nha!`;
      levelClass = 'text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-950/80 border-amber-300 dark:border-amber-700';
      playSfx('click');
    } else {
      roast = `Trời ơi! Bạn đọc "${userSpeech}" nghe như đang đọc thần chú vậy! Từ gốc là "${wordChinese}" mà! Thử lại lần nữa nào!`;
      levelClass = 'text-rose-600 dark:text-rose-300 bg-rose-50 dark:bg-rose-950/80 border-rose-300 dark:border-rose-700';
      playSfx('error');
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in">
      <div className="bg-[#fffbeb] dark:bg-slate-900 border-4 border-[#1f2937] dark:border-slate-700 rounded-3xl max-w-md w-full p-6 shadow-2xl relative text-center text-[#1f2937] dark:text-slate-100">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 w-8 h-8 rounded-full bg-rose-100 dark:bg-slate-800 hover:bg-rose-200 border-2 border-rose-400 dark:border-slate-600 text-rose-700 dark:text-rose-400 font-bold flex items-center justify-center transition cursor-pointer"
        >
          ✕
        </button>

        <span className="bg-pink-400 dark:bg-rose-600 text-pink-950 dark:text-white font-black text-xs px-3 py-1 rounded-full uppercase tracking-wider flex items-center justify-center gap-1.5 w-max mx-auto shadow">
        ...
        <div className="my-6 flex flex-col items-center justify-center">
          <button
            ...
            className={`w-20 h-20 rounded-full border-4 border-[#1f2937] dark:border-slate-600 flex items-center justify-center shadow-[3px_3px_0px_#1f2937] transition-all transform active:scale-95 cursor-pointer ${
              isRecording
        ...
        {transcribedText && (
          <div className="mb-3 p-2 bg-amber-100/70 dark:bg-slate-800 border border-amber-300 dark:border-slate-700 rounded-xl text-xs font-bold text-amber-900 dark:text-amber-200">
            Giọng nói nghe được: <span className="font-serif text-rose-600 dark:text-rose-400 font-black">"{transcribedText}"</span>
          </div>
        )}
        ...
        {evalResult ? (
          ...
          <p className="text-xs text-amber-800 dark:text-amber-300 font-medium italic">{statusMsg}</p>
        )}
      </div>
    </div>
  );
