...
async function seedEndlessOpenAI() {
  process.env.IS_TERMINAL_BOT = 'true';
  console.log('⚡==================================================⚡');
  console.log('🚀 BOT TERMINAL AI (OPENAI GPT-4o) CRAWL TỪ VỰNG HSK 1-9');
  console.log('⚡==================================================⚡\n');
  ...
  console.log(`🎲 Đang kết nối OpenAI GPT-4o-mini crawl từ vựng HSK ${randomLevel} thuộc chủ đề "${randomTheme}"...`);

  const systemPrompt = `You are a professional Chinese language teacher.
Generate 10 real, valid, authentic Chinese HSK Level ${randomLevel} vocabulary items for topic "${randomTheme}".
Requirements:
...
[
  ...
  const userPrompt = `Generate 10 genuine HSK ${randomLevel} vocabulary items for topic "${randomTheme}". Seed: ${randomSeed}. Return ONLY raw JSON array.`;
  ...
  try {
    ...
    if (Array.isArray(items)) {
      for (const item of items) {
        ...
        if (exCh.includes('<ctrl42>') || exVi.includes('Ví dụ minh họa') || exVi.includes('đây là câu ví dụ')) continue; 
    ...
    console.warn(`   ⚠️ Terminal Bot Notice: ${err.message || err}`);
  }

  const duration = ((Date.now() - startTime) / 1000).toFixed(2);
  console.log('\n==================================================');
  console.log(`🎉 NẠP THÀNH CÔNG TRONG ${duration} GIÂY!`);
  console.log(`✨ ĐÃ NẠP MỚI TỔNG CỘNG: +${totalAdded} TỪ VỰNG CHUẨN XÁC!`);
  console.log(`📚 TỔNG KHO TỪ VỰNG HSK BÂY GIỜ LÀ: ${existingSet.size} TỪ.`);
  console.log('==================================================\n');
}

seedEndlessOpenAI().then(() => process.exit(0)).catch(e => { console.error(e); process.exit(1); });
