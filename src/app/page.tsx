...
export default function GamePage() {
    ...
    const restoreSession = async () => {
      ...
      if (errorCode) {
        const messages: Record<string, string> = {
          ...
          account_link_conflict: 'Email này đã liên kết với tài khoản Google khác.',
        };
        setAuthError(messages[errorCode] ?? 'Không thể đăng nhập bằng Google. Vui lòng thử lại.');
      ...
      try {
        ...
        setUser(data.user);
        await fetchProgress(data.user.id);
      } catch {
  ...
  const handleLogout = async () => {
    try { await fetch('/api/logout', { method: 'POST' }); } catch { /* Clear local state below. */ }
    localStorage.removeItem('boba_game_user_id');
    localStorage.removeItem('boba_game_username');
    localStorage.removeItem('boba_game_email');
    setUser(null);
    ...
    return (
      <main className="min-h-screen bg-[#fffdf8] flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-[#fffaf0] border-[3px] border-[#1f2937] shadow-[8px_8px_0px_#1f2937] rounded-2xl p-6 relative overflow-hidden">
          ...
          <a href="/api/auth/google/start" className="mb-5 flex w-full items-center justify-center gap-2 rounded-lg border-2 border-[#1f2937] bg-white p-3 text-sm font-black text-[#111827] shadow-[3px_3px_0px_#1f2937] transition-transform hover:-translate-y-0.5">
          ...
          <div className="mb-5 flex items-center gap-3 text-[10px] font-black uppercase text-[#5b6474]"><span className="h-px flex-1 bg-[#9ca3af]" /><span>hoặc</span><span className="h-px flex-1 bg-[#9ca3af]" /></div>
